...
$(document).ready(function() {
    $('button[data-id="cartQuantity"]').click(function(event) {
      var prevInpt = $(this).prev();
      var newtInpt = $(this).next();
  
      if(prevInpt.length) {  prevInpt[0].value++; prevInpt.change();}
      if(newtInpt.length) {  newtInpt[0].value--; newtInpt.change();}


    });
    
    $('.buttonMinus').click(function(event) {
        var timeval = --$('input[data-id="' + $(this)[0].dataset.id + '"]')[0].value;
        if (timeval < 1) timeval = 1;
        $('input[data-purpose="' + $(this)[0].dataset.id + '"]')[0].value = timeval;
    });
    $('.buttonPlus').click(function(event) {
        var timeval = ++$('input[data-id="' + $(this)[0].dataset.id + '"]')[0].value;
        $('input[data-purpose="' + $(this)[0].dataset.id + '"]')[0].value = timeval;
    });
    $('input[data-id]').change(function(event) {
        $('input[data-purpose="' + $(this)[0].dataset.id + '"]')[0].value = $(this)[0].value < 1 ? 1 : $(this)[0].value;
    });
